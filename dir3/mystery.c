...
int main (int argc, char **argv)
{
  ...
  if(verbose && rank ==0)
  {
     ...
     printf ("Each rank writes 'several' blocks of size %d bytes to a file\n", block_size);
  }
  ...
  int rankm=rank%10;
   ...
   /* Generate an array of offset for 1K random write access to the file
    * 
    */
    int *offset_range;
  ...
  int error = MPI_File_open(MPI_COMM_WORLD, path, MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &fh);
    ...
    /*
     *  Nice sequential I/O pattern  
     */
    MPI_Offset disp = rank*block_size;
    MPI_File_write_at(fh, disp , fill, block_size, MPI_CHAR, MPI_STATUS_IGNORE);

    /* Bad 1K random I/O pattern
     * offset is the base offset for the current process
     * plus the index in the permutation array (expressed in KB)
     */
    for(int i = 0 ; i < block_size / 1024; i++)
    {
        disp = (size + rank)*block_size + offset_range[i] * 1024;
        MPI_File_write_at(fh, disp , fill, 1024, MPI_CHAR, MPI_STATUS_IGNORE);
    }

    /*
     *  Nice sequential I/O pattern  
     */
    disp = (2 * size + rank)*block_size;
    MPI_File_write_at(fh, disp , fill, block_size, MPI_CHAR, MPI_STATUS_IGNORE);
