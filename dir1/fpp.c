...
int main (int argc, char **argv){
  ...
  if(verbose && rank ==0)
  {
     ...
     printf ("Each rank writes a block of size %d bytes to a file\n", block_size);
  }
  ...
  sprintf(path, "%s_%d", workspace,rank);
  ...
  int error = MPI_File_open(MPI_COMM_SELF, path, MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &fh);
